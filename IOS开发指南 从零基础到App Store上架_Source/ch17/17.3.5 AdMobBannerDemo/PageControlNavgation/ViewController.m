...
- (void)viewDidLoad
{
    ...
    float height = result.height - self.pageControl.frame.size.height - statusBarHeight;
    
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {//判断是iOS 7系统
        height = result.height - self.pageControl.frame.size.height;
    }
    
	self.scrollView.contentSize = CGSizeMake(4 * result.width, height);
...
- (GADRequest *)createRequest {
    ...
    request.testDevices = [NSArray arrayWithObjects:
                           @"C09D1203-4B49-4339-BBF5-D01FDE8E0470", nil];
#endif
